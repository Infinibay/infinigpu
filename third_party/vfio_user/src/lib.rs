...
impl Server {
    ...
    fn handle_command(
        ...
        match header.command {
            ...
            Command::SetIrqs => {
                ...
                // infinigpu vendored patch: SET_IRQS with VFIO_IRQ_SET_DATA_BOOL appends `count`
                // inline bool bytes after the fixed struct (message_size = size_of::<SetIrqs>() +
                // count). Upstream reads only the struct and early-returns on the range/DATA_BOOL
                // checks below WITHOUT consuming those trailing bytes, desyncing the stream. Drain
                // the declared remainder first so every return path stays aligned.
                let trailing =
                    (cmd.header.message_size as usize).saturating_sub(size_of::<SetIrqs>());
                if trailing > 0 {
                    let mut sink = vec![0u8; trailing];
                    stream.read_exact(&mut sink).map_err(Error::StreamRead)?;
                }
            ...
            Command::RegionWrite => {
                ...
                // infinigpu vendored patch: honor a POSTED (NO_REPLY) write. QEMU 10.1 pipelines
                // MMIO REGION_WRITEs with the NO_REPLY flag and does NOT wait for a reply — so a
                // reply it never expected is read as "unexpected reply", and its bytes misalign the
                // next header ("bad header size"), breaking the connection. Upstream always replies.
                // When NO_REPLY is set we process the write and send nothing.
                if cmd.header.flags & (HeaderFlags::NoReply as u32) == 0 {
                    let reply = RegionAccess {
                        header: Header {
                            message_id: cmd.header.message_id,
                            command: Command::RegionWrite,
    ...
    pub fn run(&self, backend: &mut dyn ServerBackend) -> Result<(), Error> {
        ...
        loop {
            ...
            // --- infinigpu vendored patch: unsupported-command stream re-alignment ---
            // Upstream handle_command returns an error for unsupported/unknown opcodes WITHOUT
            // draining the request body, which misaligns the socket stream — every subsequent
            // header is then read from leftover body bytes (QEMU: "bad header size" /
            // "unexpected reply"), breaking the connection. QEMU 10.1 sends opcodes this crate
            // doesn't know (DEVICE_FEATURE=16 for error recovery, GET_REGION_IO_FDS, dirty-page
            // tracking), so this bit every real guest boot. Reading `command` as an enum is also
            // UB when the wire opcode isn't a valid discriminant, so decode it as a raw u16 and
            // intercept anything handle_command wouldn't process: drain the body, reply with an
            // errno that echoes the opcode, and keep the stream aligned for the next command.
            let raw_cmd = {
                let b = header.as_slice();
                u16::from_ne_bytes([b[2], b[3]])
            };
            debug!(
                "recv command raw={raw_cmd} size={} id={} flags={:#x}",
                header.message_size, header.message_id, header.flags
            );
            // Opcodes handle_command actually services (see the match in handle_command).
            const SUPPORTED: &[u16] = &[1, 2, 3, 4, 5, 7, 8, 9, 10, 13];
            if !SUPPORTED.contains(&raw_cmd) {
                let body =
                    (header.message_size as usize).saturating_sub(size_of::<Header>());
                if body > 0 {
                    let mut sink = vec![0u8; body];
                    stream.read_exact(&mut sink).map_err(Error::StreamRead)?;
                }
                let mut reply = Header {
                    message_id: header.message_id,
                    command: Command::Unknown,
                    // A valid vfio-user reply MUST set the Reply bit; an error reply is Reply|Error.
                    // (Upstream sets only Error, which QEMU 10.1 rejects as "unexpected reply".)
                    flags: HeaderFlags::Reply as u32 | HeaderFlags::Error as u32,
                    message_size: size_of::<Header>() as u32,
                    error: EINVAL as u32,
                };
                // Echo the original opcode so QEMU matches the reply to its request.
                reply.as_mut_slice()[2..4].copy_from_slice(&raw_cmd.to_ne_bytes());
                stream
                    .write_all(reply.as_slice())
                    .map_err(Error::StreamWrite)?;
                debug!("replied EINVAL to unsupported command raw={raw_cmd} (stream realigned)");
                continue;
            }
            // --- end vendored patch ---
            ...
            if let Err(e) = self.handle_command(backend, &mut stream, header, fds) {
                ...
                let reply = Header {
                    ...
                    // Must set the Reply bit too, or QEMU rejects it as "unexpected reply".
                    flags: HeaderFlags::Reply as u32 | HeaderFlags::Error as u32,
                    message_size: size_of::<Header>() as u32,
