...
pub struct ForwardedCmdListTail {
    ...
    /// (e.g. an MVP matrix at binding 0 read by the VS + a material/colour block at binding 4 read by
    /// the FS) — zink's fixed-function path binds ≥2, and dropping any collapses `gl_Position`.
    /// (ABI ≤0.13 carried a single raw UBO here + its binding in the field now named `ubo_count`.)
    pub ubo_len: u32,
